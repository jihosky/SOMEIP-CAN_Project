...
## Milestone 2C event test

`test_someip_events.py` starts the integrated gateway, waits for the client to subscribe through vSomeIP Service Discovery, and starts the virtual ECU in the acceleration scenario. It verifies three event values and their deterministic order: 20/1200/70, 40/1800/71, and 60/2400/72. It also applies the existing bounded shutdown cleanup.

The event codec unit test verifies the eight-byte big-endian combined payload and rejects an incorrect payload length. The Python tests verify the exact three CAN payloads generated by the acceleration scenario. The integration test is marked `integration`; it requires an existing vCAN interface and permission for vSomeIP UDP multicast and local sockets. It skips when the selected CAN interface is absent.
